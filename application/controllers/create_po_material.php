...
class Create_po_material extends CI_Controller{
	function __construct(){
		parent::__construct();
		$this->load->library('render');
		$this->load->model('create_po_material_model');
		$this->load->library('access');
		$this->access->set_module('create_po_material_mocel.create_po_material_mocel');
		$this->access->user_page();
	}
	
	function index(){
		
		$this->render->add_view('app/create_po_material/list');
		$this->render->build('Create po bahan');
		$this->render->show('Create po bahan');
		
	}

	
	function table_controller(){
		$data = $this->create_po_material_model->list_controller();
		send_json($data);
	}
	function form($id = 0){
		$data = array();
		if($id==0){
			...
			$data['row_id'] = '';
			$data['tpm_code'] = format_code('transaction_po_materials','tpm_code','POB',7);
			$data['stand_id'] ='';
			...
			$result = $this->create_po_material_model->read_id($id);
			if($result){
				$data = $result;
				$data['row_id'] = $id;
		...
		$this->render->add_form('app/create_po_material/form', $data);
		$this->render->build('Create po bahan');
		
		if($id==0){
			$this->render->add_view('app/create_po_material/transient_list');
			$this->render->build('Data Parts');
		}else{
			$this->render->add_view('app/create_po_material/transient_list_view');
			$this->render->build('Data Parts');
		}

	
		$this->render->add_form('app/create_po_material/form_save', $data);
		$this->render->build('Create po bahan');
		$this->render->show('Create po bahan');
	}
	...
	function form_action($is_delete = 0) // jika 0, berarti insert atau update, bila 1 berarti delete
	{
		...
		if($is_delete)
		{
			...
			$is_process_error = $this->create_po_material_model->delete($id);
		...
		$data['tpm_type']		= 1;//1 untuk type materaial bahan
		...
		foreach($list_tm_stock_id as $key => $value)
		{
			...
			if($check > 1){
				...
				$get_data_materails = $this->create_po_material_model->get_data_material($tm_stock_id_original);
	...
	//	
		...
		{ 
			$error = $this->create_po_material_model->create($data, $items);
	...
	function detail_table_loader($id = 0) {
		...
		$data = $this->create_po_material_model->detail_list_loader($id);
		$sort_id = 0;
	...
	function detail_form($id = 0) // jika id tidak diisi maka dianggap create, else dianggap edit
		{		
			...
			$this->load->helper('form');
			$this->render->add_form('app/create_po_material/transient_form', $data);
	...
	function load_satuan()
	{
		...
		$query = $this->create_po_material_model->load_satuan($id);
		$data = array();
