<?php
class po_received_model extends CI_Model 
{
	var $trans_type = 5;
	var $insert_id = NULL;
	...
	{		
		...
		foreach($query->result_array() as $row) {
			...
			$link = "<a href=".site_url('po_received/form/'.$row['tp_id'])." class='link_input'> Received </a>";	
	...
	function update($id,$items)
	{
		...
		foreach($items as $row)
		{			
			$data_up['tpd_detail_received'] = $row['tpd_detail_received'];
			$this->db->where('tpd_id',$row['tpd_id']);
			$this->db->update('transaction_po_details',$data_up);
			//update status parts	
			
				//create history parts
				$data_history['tpd_id']		 =	$row['tpd_id'];
				$data_history['tpdh_type'] =	$row['tpdh_type'];
				$data_history['tpdh_qty']  = 	$row['tpdh_qty'];	
				$data_history['tpdh_date'] =	$row['tpdh_date'];
				$data_history['tpdh_desc']  = 	$row['tpdh_desc'];	
