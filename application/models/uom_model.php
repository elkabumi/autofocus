...
class Unit_model extends CI_Model 
{
	...
	{		
		...
		$this->db->select('a.*, b.employee_name as created_name, c.employee_name as inactive_name',
							 1);
		$this->db->from('unit a');
		$this->db->join('employees  b','a.created_by_id  = b.employee_id','left');
		$this->db->join('employees  c','a.inactive_by_id = c.employee_id','left');
		//query();
		$query = $this->db->get();
		foreach($query->result_array() as $row)
		{
			...
			$unit_date = format_new_date($row['unit_date']);
			$active = show_checkbox_status($row['unit_active_status']);
				
			if($row['unit_active_status'] == 0){
				
				
				$div1 = "<span class='inactive'>";
				$div2 = "</div>";
				$row['unit_id'] = $row['unit_id'];
				$row['unit_name'] = $div1.$row['unit_name'].$div2;
				$unit_date = $div1.$unit_date.$div2;
	...
	function create($data)
	{
		...
		$this->access->log_insert($id, "Uom [$data[unit_name]]");
	...
	function update($id, $data)
	{
		...
		$this->access->log_update($id, "Uom [$data[unit_name]]");
